...
class FobosSDRAgileSourceModule : public ModuleManager::Instance {
public:
    FobosSDRAgileSourceModule(std::string name) {
        this->name = name;
...
private:
    ...
    static void start(void* ctx) {
        ...
        fobos_sdr_set_samplerate(_this->openDev, (_this->sampleRate >= 50e6) ? _this->sampleRate : 50e6);
...
MOD_EXPORT ModuleManager::Instance* _CREATE_INSTANCE_(std::string name) {
    return new FobosSDRAgileSourceModule(name);
}

MOD_EXPORT void _DELETE_INSTANCE_(void* instance) {
    delete static_cast<FobosSDRAgileSourceModule*>(instance);
}
